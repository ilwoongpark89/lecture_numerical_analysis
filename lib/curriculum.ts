...
// HT 골드 레퍼런스와 동일한 "챕터 × 8단계" 코스웨어 구조.

// 모든 콘텐츠 챕터가 밟는 8단계 (HT 정합). 정적 강의의 data-stage + 어드민 맵 셀 + na-track dwell.
...
export type Week = {
  ...
  ready: boolean;
  exam?: boolean;
  chapters: Chapter[]; // 콘텐츠 챕터 (시험/미완성 주차는 [])
};
...
export const WEEKS: Week[] = [
  ...
  },
  {
    ...
    chapters: C(2, [[1, "MATLAB 환경과 문법"], [2, "벡터·행렬 연산"], [3, "제어 흐름과 함수"], [4, "그래프 시각화"]]),
  },
  {
...
export const CURRICULUM: WeekDef[] = WEEKS.filter((w) => w.chapters.length > 0).map((w) => ({
  week: w.week,
  title: w.title,
