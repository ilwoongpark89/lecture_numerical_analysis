    ...
    <main className="min-h-screen bg-slate-950">
      <nav className="fixed top-0 left-0 right-0 z-50 bg-slate-950/80 backdrop-blur-md border-b border-slate-800">
        <div className="container mx-auto px-4 h-14 flex items-center justify-between">
          ...
          <span className="text-sm text-gray-500">Week 7 — Iterative Methods</span>
        </div>
      </nav>
      ...
      <div className="pt-14">
        <section className="relative min-h-[60vh] flex items-center justify-center overflow-hidden bg-slate-950">
          <div className="absolute inset-0 z-[1]">
            <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-sky-500/15 rounded-full blur-3xl opacity-60 translate-x-1/3 -translate-y-1/3" />
            <div className="absolute bottom-0 left-0 w-[600px] h-[600px] bg-blue-600/15 rounded-full blur-3xl opacity-60 -translate-x-1/3 translate-y-1/3" />
          </div>
          <div className="container relative z-10 mx-auto px-4 text-center">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="mt-10 flex flex-wrap justify-center gap-3 text-sm">
              {[
                { label: "Jacobi 반복법", icon: "🔄" },
                { label: "Gauss-Seidel", icon: "⚡" },
                { label: "수렴 조건", icon: "📊" },
                { label: "SOR", icon: "🚀" },
                { label: "실습", icon: "🧪" },
                { label: "Python", icon: "🐍" },
          ...
          <div className="container mx-auto px-4 text-center">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              ...
              <h3 className="text-2xl md:text-3xl font-bold text-white mb-6">
                Week 8: <span className="text-transparent bg-clip-text bg-gradient-to-r from-gray-400 to-slate-400">Mid-term Exam</span>
              </h3>
              <p className="text-gray-400 mb-8 max-w-xl mx-auto">Weeks 1-7 범위의 중간고사입니다</p>
            </motion.div>
          </div>
        </section>
      </div>
