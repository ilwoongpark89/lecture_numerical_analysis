...
- ✅ 3.8 float 범위/최소양수 — 3-1 수직선(ULP 간격)+머신엡실론 원리로 충분
...
- ✅ 5.2 짝수 근 브래킷 — w4-2 (funcplot (x-1)(x-2), 같은부호 근2개)
- ✅ 5.3 예외(중근 접촉=부호변화 없음) — w4-1 (funcplot (x-1.5)²)
- ✅ 5.8 브래킷 경우 — 5.2/5.3 로 충분(부호판정 함정 커버)
...
- ✅ 6.8 가위치 vs 할선 — 양 방법 각 기하 그림(5-1 할선+5 가위치) + 구분 서술로 커버
- ➖ 6.11 Muller/역2차 (고급, 범위밖)
...
- ✅ 수렴 — w7-3 (convergence)
- ✅ 11.5 GS 수렴/발산 — w7-3 convergence + w5 cobweb 개념으로 커버(2D cobweb 은 표현 과부하)
...
- ✅ 17.2 왜 제곱합인가 — w9-1 (funcplot 두 직선 Σe=0)
- ➖ 17.14 다중회귀 3D 평면 (고급; 정규방정식 (XᵀX)a=Xᵀy 는 9-2 서술)
...
- ✅ 23.4/23.6 응용(온도·거리) — 23.7 잡음 + diff 스테퍼로 실측미분 개념 커버
...
- ✅ 29.4 Dirichlet 경계 판 — w14 가열판 matrixgrid(경계 고정·내부 미지) 로 커버
- ➖ 30.10/30.11 ADI (고급)
...
- ✅ 27.6 진동 모드 — w15-1 (funcplot sin nπx = 모드 모양 = 고유벡터)
